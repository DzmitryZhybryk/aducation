...
5. Зачем нужны индексы?
    - `Если мы по какому-то полю работаем часто, т.е. часто делаем поиск по нему, есть смысл проиндексировать это поле. Работа с этим полем будет быстрее`
...
9. Уровни изолированности транзакции
    - `SERIALIZABLE` - самый высокий уровень изолированности. Транзакции полностью изолируют друг другу
    - `REPEATABLE READ` - Уровень, при котором читающая транзакция «не видит» изменения данных, которые были ею ранее
      прочитаны. При этом никакая другая транзакция не может изменять данные, читаемые текущей транзакцией, пока та не
      окончена
    - `READ COMMITTED` - в процессе работы одной транзакции другая может быть успешно завершена и сделанные ею изменения
      зафиксированы
