...
8. Что такое ACID
    - `Это про то, что наши транзакции надёжны`
    - `A` - atomicity/атомарность, транзакция не может быть зафиксированна частично
    ...
    - `D` - durability/прочность, если транзакция завершена, то результат зафиксирован в системе и никакой внешний
      фактор на это не может повлиять
9. Уровни изолированности транзакции
    - `SERIALIZABLE` - На этом уровне моделируется последовательное выполнение всех зафиксированных транзакций, как если
      бы транзакции выполнялись одна за другой, последовательно, а не параллельно
    - `REPEATABLE READ` - Этот уровень отличается от Read Committed тем, что запрос в транзакции данного уровня видит
      снимок данных на момент начала первого оператора в транзакции (не считая команд управления транзакциями), а не
      начала текущего оператора. Таким образом, последовательные команды SELECT в одной транзакции видят одни и те же
      данные; они не видят изменений, внесённых и зафиксированных другими транзакциями после начала их текущей
      транзакции.
    - `READ COMMITTED` - В транзакции, работающей на этом уровне, запрос SELECT (без предложения FOR UPDATE/SHARE) видит
      только те данные, которые были зафиксированы до начала запроса; он никогда не увидит незафиксированных данных или
      изменений, внесённых в процессе выполнения запроса параллельными транзакциями
      зафиксированы. Однако SELECT видит результаты изменений, внесённых ранее в этой же транзакции, даже если они ещё
      не зафиксированы. Также заметьте, что два последовательных оператора SELECT могут видеть разные данные даже в
      рамках одной транзакции, если какие-то другие транзакции зафиксируют изменения после запуска первого SELECT, но до
      запуска второго.
    - `READ UNCOMMITTED` - низший уровень. Транзакции, выполняющие только чтение, при данном уровне изоляции никогда не
      блокируются.
10. Как создать таблицу?
    - `CREATE TABLE IF NOT EXIST test_table (first_column INT, second_column VARCHAR(255))`
11. Как изменить таблицу?
...
14. Типы JOIN при SELECT?
    ...
    - `full join` - SELECT column_name FROM table_1 FULL OUTER JOIN ON table_1.column_name = table_2.column_name WHERE
      condition
15. Отличие REPEATABLE READ от SELECT FOR UPDATE?
...
16. Виды блокировок базы данных:
...
17. Что такое нормализация и де-нормализация базы данных?
...
18. Какие типы нормализации бывают?
...
1. Какие бывают паттерны проектирования?
    ...
    - `Поведенческие`
        ...
        - `шаблонный метод` - паттерн предлагает разбить некоторый алгоритм на последовательность шагов, описать эти
          ...
          Дополнительно: композиция, репозиторий

## Сетевая модель OSI

1. Что представляет из себя модель OSI
    - `Прикладной уровень` - использует свои протоколы, чтобы пользователь увидел данные. HTTP,
    - `Уровень представления` -
    - `Сеансовый уровень` - RPC, gRPC
    - `Транспортный уровень` - занимается доставкой пакетов. Представлен TCP и UPD
    - `Сетевой уровень` - объединяет локальную и глобальную сеть при помощи маршрутизаторов. IP позволяет понять, в
      какой подсети находится хост. Маска позволяет понять, где находится номер подсети, а где хост
